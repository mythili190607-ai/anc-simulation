...
Laptop/Desktop version - No GPIO or sounddevice required
FIXED VERSION - Works out of the box
"""
...
class DisplayThread(threading.Thread):
    """Real-time display of ANC metrics"""
    def __init__(self):
    ...
    def update(self, label, error_db, reduction_db):
        self.label = label
    ...
    def run(self):
        while not self._stop_event.is_set():
            sys.stdout.write(f"\r[ANC] Label: {self.label:15} | Error: {self.error_db:6.2f} dB | Reduction: {self.reduction_db:6.2f} dB")
            sys.stdout.flush()
            time.sleep(0.1)
        print()
...
class ClassifierThread(threading.Thread):
    def __init__(self, samplerate, window_ms=150.0, smoothing=0.35, hold_count=3):
        ...
        self.in_queue = queue.Queue(maxsize=32)
    ...
    def submit(self, reference_block):
        try:
            block = np.asarray(reference_block, dtype=np.float64).reshape(-1).copy()
            self.in_queue.put_nowait(block)
        except queue.Full:
            pass
...
def build_arg_parser():
    p = argparse.ArgumentParser(description="Real-time multi-algorithm ANC Simulator (Fixed - No sounddevice needed)")
    p.add_argument("--samplerate", type=int, default=DEFAULTS["samplerate"])
    p.add_argument("--block-ms", type=float, default=DEFAULTS["block_ms"])
    p.add_argument("--filter-len", type=int, default=DEFAULTS["filter_len"])
...
def validate_args(args):
    if args.samplerate <= 0:
        raise SystemExit("--samplerate must be greater than 0.")
    if args.block_ms <= 0:
        raise SystemExit("--block-ms must be greater than 0.")
    if args.filter_len < 2:
        raise SystemExit("--filter-len must be at least 2.")
    if args.duration <= 0:
...
def main():
    ...
    print("=" * 60)
    print("ANC SIMULATOR - FIXED VERSION (No sounddevice required)")
    print("=" * 60)
    ...
    print(f"\n[1/3] Generating {args.noise_type} noise...")
    ...
    # Simulate secondary path effect
    ...
    # Process audio
    print(f"\n[2/3] Processing {args.duration}s of audio at {args.samplerate}Hz...")
    ...
    for i in range(0, total_samples, blocksize):
        ...
        for j, sample_ref in enumerate(block_ref):
            if i + j < end_idx:
                ...
                (anti_fx, anti_vs, anti_rb), (err_fx, err_vs, err_rb) = engine.process_sample(sample_ref, sample_err)

                # Combine algorithms with learned weights
                combined_anti = (classifier.weights[0] * anti_fx +
                               classifier.weights[1] * anti_vs +
                               classifier.weights[2] * anti_rb)

                output_signal[i + j] = combined_anti
                error_signal[i + j] = sample_err - combined_anti

        times.append(i / args.samplerate)
        labels_history.append(classifier.label)

        if args.enable_display and i % (blocksize * 10) == 0:
            ref_rms = np.sqrt(np.mean(block_ref ** 2))
            err_rms = np.sqrt(np.mean(error_signal[max(0, i-blocksize):i+blocksize] ** 2))

            ref_db = 20 * np.log10(ref_rms + 1e-10)
            err_db = 20 * np.log10(err_rms + 1e-10)
            reduction = ref_db - err_db

            display.update(classifier.label, err_db, reduction)

    elapsed = time.time() - start_time
    print(f"\n\n[3/3] Processing complete in {elapsed:.2f}s")
    ...
    print(f"RESULTS")
    print(f"{'=' * 60}")
    print(f"  Reference RMS: {ref_db:.2f} dB")
    print(f"  Error RMS: {err_db:.2f} dB")
    ...
    if args.save_plot:
        ...
        axes[0].plot(t, reference, label='Reference', alpha=0.7, color='blue', linewidth=1.5)
        axes[0].plot(t, measured_error, label='Measured Error', alpha=0.7, color='orange', linewidth=1.5)
        axes[0].set_ylabel('Amplitude')
        axes[0].set_title('Input Signals')
        axes[0].legend(loc='upper right')
        axes[0].grid(True, alpha=0.3)

        axes[1].plot(t, output_signal, label='Anti-Noise Output', alpha=0.7, color='green', linewidth=1.5)
        axes[1].set_ylabel('Amplitude')
        axes[1].set_title('Generated Anti-Noise')
        axes[1].legend(loc='upper right')
        axes[1].grid(True, alpha=0.3)

        axes[2].plot(t, error_signal, label='Residual Error', alpha=0.7, color='red', linewidth=1.5)
